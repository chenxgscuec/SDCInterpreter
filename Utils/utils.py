...
def get_node_map_dict():
    nodes_path = './data/nodes/'
    node_drug1 = read_dict(nodes_path + 'drug_1_dict.pkl')  # gene symbol
    node_drug2 = read_dict(nodes_path + 'drug_2_dict.pkl')  # drug_name
    node_cell = read_dict(nodes_path + 'cell_dict.pkl')
    return node_drug1, node_drug2, node_cell

def df_node_remap(drug_combination_df ,node_drug1:dict, node_drug2:dict, node_cell:dict, node_drug12:dict):
    drug_combination_df['drug1_db'] = drug_combination_df['drug1_db'].map(node_drug1)
    drug_combination_df['drug2_db'] = drug_combination_df['drug2_db'].map(node_drug2)
    ...
    drug_combination = drug_combination_df[['drug1_db', 'drug2_db', 'Drug12', 'cell', 'synergy']]
    return drug_combination

def drug_combination_classification(drug_combination:pd.DataFrame, threshold:float):
    drug_combination = np.array(drug_combination).astype(int)
    for sample in drug_combination:
        sample[4] = 1 if sample[4] >= threshold else 0
    num_pos = np.sum(drug_combination[:, 4] == 1)
    print('drug_combination # pos_sample: {0}, # neg_sample: {1}, # all_sample: {2}'.format(num_pos, len(drug_combination)-num_pos, len(drug_combination)))
    return drug_combination
...
class Data(object):
    def __init__(self,):
        ...
        node_cell = read_dict(nodes_path + 'cell_dict.pkl')
        drug_combination_df = pd.read_csv('./Data/DrugCombDB/drug_combinations.csv')
        drug_combination = df_node_remap(drug_combination_df, node_drug1, node_drug2, node_cell, node_drug12)
        graph_list, _ = dgl.load_graphs('Data/graph/graph_drugcombdb')
        self.graph_all = graph_list[0]
        self.drug_synergy = drug_combination_classification(drug_combination, threshold=0)
...
'''
DGL graph_1 manipulation utils
'''


def get_homo_nids_to_hetero_nids(ghetero):
    '''
    Create a dictionary mapping the node ids of the homogeneous version of the input graph_1
    to the node ids of the input heterogeneous graph_1.

    Parameters
    ----------
    ghetero : heterogeneous dgl graph_1
...
def get_homo_nids_to_ntype_hetero_nids(ghetero):
    '''
    Create a dictionary mapping the node ids of the homogeneous version of the input graph_1
    to tuples as (node type, node id) of the input heterogeneous graph_1.
...
def get_ntype_hetero_nids_to_homo_nids(ghetero):
    '''
    Create a dictionary mapping tuples as (node type, node id) of the input heterogeneous graph_1
    to the node ids of the homogeneous version of the input graph_1.
...
def get_num_nodes_dict(ghetero):
    '''
    Create a dictionary containing number of nodes of all ntypes in a heterogeneous graph_1
    Parameters
    ----------
    ghetero : heterogeneous dgl graph_1
...
def remove_all_edges_of_etype(ghetero, etype):
    ...
    Returns
    ----------
    removed_ghetero : heterogeneous dgl graph_1
...
def hetero_src_tgt_khop_in_subgraph(src_ntype, src_nid, tgt_ntype, tgt_nid,drug1_nid,drug2_nid, ghetero, k):
    '''
    ...
    sghetero : heterogeneous dgl graph_1
    ...
    drug1_nid = drug1_nid.item() if torch.is_tensor(drug1_nid) else drug1_nid
    drug2_nid = drug2_nid.item() if torch.is_tensor(drug2_nid) else drug2_nid
    ...
    if src_ntype == tgt_ntype:
        ...
        pred_dict = {src_ntype: src_nid, tgt_ntype: tgt_nid, 'drug1': drug1_nid, 'drug2': drug2_nid}
        # pred_dict = {src_ntype: src_nid, tgt_ntype: tgt_nid}

        sghetero, inv_dict = khop_in_subgraph(ghetero, pred_dict, k)
        sghetero_src_nid = inv_dict[src_ntype]
        sghetero_tgt_nid = inv_dict[tgt_ntype]
        sghetero_drug1_nid = inv_dict['drug1']
        sghetero_drug2_nid = inv_dict['drug2']

    sghetero_feat_nid = sghetero.ndata[dgl.NID]

    return sghetero_src_nid, sghetero_tgt_nid,sghetero_drug1_nid, sghetero_drug2_nid,sghetero, sghetero_feat_nid
...
def get_neg_path_score_func(g, weight, exclude_node=[]):
    ...
    u, v = g.edges()
    neg_path_score_map = {edge: log_in_degrees[edge[1]] - log_eweights[i] for i, edge in
                          enumerate(zip(u.tolist(), v.tolist()))}
...
def k_shortest_paths_generator(g,
    ...
    """Generate at most `k` simple paths in the graph_1 g from src_nid to tgt_nid,
...
def get_comp_g_edge_labels(comp_g, edge_labels):
    """Turn `edge_labels` with node ids in the original graph_1 to
       `comp_g_edge_labels` with node ids in the computation graph_1.
       For easier evaluation.

    Parameters
    ----------
    comp_g : heterogeneous dgl graph_1
        computation graph_1, with .ndata stores key dgl.NID
    ...
    for can_etype in edge_labels:
        ...
        start_comp_g_max_nid, end_comp_g_max_nid = ntypes_to_comp_g_max_nids[start_ntype], ntypes_to_comp_g_max_nids[
            end_ntype]
...
def get_comp_g_path_labels(comp_g, path_labels):
    """Turn `path_labels` with node ids in the original graph_1
       `comp_g_path_labels` with node ids in the computation graph_1
       For easier evaluation.
    ...
    for path in path_labels:
        ...
        for can_etype, start_nid, end_nid in path:
            start_ntype, etype, end_ntype = can_etype
            start_comp_g_max_nid, end_comp_g_max_nid = ntypes_to_comp_g_max_nids[start_ntype], \
            ntypes_to_comp_g_max_nids[end_ntype]
...
def plot_hetero_graph(ghetero,
    ...
    Parameters
    ----------
    ghetero: a DGL heterogeneous graph_1 with ndata `order`
    ...
    # Convert DGL graph_1 to networkx graph_1
    ghomo = dgl.to_homogeneous(ghetero)
    ...
    for ntype in ghetero.ntypes:
        ...
        hetero_nids = ghomo.ndata[dgl.NID]  # nid in the original hetero graph_1
        ...
        # For the current node type, get node ids and prediction node id in the original hetero graph_1
