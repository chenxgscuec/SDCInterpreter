...
def remove_edges_of_high_degree_nodes(ghomo, max_degree=10, always_preserve=[]):
    ...
    Returns
    -------
    low_degree_ghomo : dgl homogeneous graph_1
        Pruned graph_1 with edges of high degree nodes removed
...
def remove_edges_except_k_core_graph(ghomo, k, always_preserve=[]):
    ...
    Returns
    -------
    k_core_ghomo : dgl homogeneous graph_1
        The k-core graph_1
    '''
    k_core_ghomo = ghomo
...
class PaGELink(nn.Module):
    ...
    model : nn.Module
        ...
        * The required arguments of its forward function are source node id, target node id,
          graph_1, and feature ids. The feature ids are for selecting input node features.
    ...
    def _prune_graph(self, ghetero, prune_max_degree=-1, k_core=8, always_preserve=[]):
        ...
        # The pruning is computed on the homogeneous graph_1, i.e., ignoring node/edge types
        ...
        # Apply the pruning result on the heterogeneous graph_1
    ...
    def get_edge_mask(self,
        ...
        homo_drug1_nid = ntype_hetero_nids_to_homo_nids[('drug1', int(drug1_nid))]
        homo_drug2_nid = ntype_hetero_nids_to_homo_nids[('drug2', int(drug2_nid))]
        ...
        #pair = torch.tensor(np.array([[drug1_id.item(), drug2_id.item(), tgt_id.item()]])).to(device)
        ...
        for e in range(self.num_epochs):
            ...
            score = self.model(ml_ghetero, pair.to(device), feat_nids, ml_eweight_dict)
    ...
    def explain(self,
        ...
        num_hops : int
            Number of hops to extract the computation graph_1, i.e. GNN # layers

        prune_max_degree : int
            If positive, prune the edges of graph_1 nodes with degree larger than `prune_max_degree`
            If  -1, do nothing

        k_core : int
            k for the the k-core graph_1 extraction
        ...
        (optional) edge_mask_dict : dict
            key=`etype`, value=torch.nn.Parameter with size being the number of `etype` edges
        """
        # Extract the computation graph_1 (k-hop subgraph)
        (comp_g_src_nid,
         comp_g_tgt_nid,
         comp_g_drug1_nid,
         comp_g_drug2_nid,
         ...
         comp_g_feat_nids) = hetero_src_tgt_khop_in_subgraph(self.src_ntype,
                                                             ...
                                                             ghetero,
                                                             num_hops)
        g1 = comp_g_feat_nids.get('gene1').tolist()
        g2 = comp_g_feat_nids.get('gene2').tolist()
        # (comp_g_src_nid,
        #  comp_g_tgt_nid,
        #  comp_g,
        #  comp_g_feat_nids,
        ...
        if comp_g.num_edges() <= 0:
            ...
            # # Learn the edge mask on the computation graph_1
            ...
            # # Covert the node id in computation graph_1 to original graph_1
